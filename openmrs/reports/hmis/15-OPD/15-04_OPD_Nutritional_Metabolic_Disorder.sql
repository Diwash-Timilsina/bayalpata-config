...
FROM
(SELECT 
    CASE WHEN concept_full_name IN ('Unspecified diabetes mellitus','Diabetic Retinopathy',
'Type 1 diabetes mellitus with ketoacidosis','Diabetes mellitus arising in pregnancy',
'Type 1 diabetes mellitus','Type 2 diabetes mellitus','Malnutrition-related diabetes mellitus',
'Other specified diabetes mellitus')
	THEN 'Diabetes Mellitus (DM)' ELSE concept_full_name END AS answer_name,
    CASE WHEN icd10_code IN ('E14','E14.3†','E10.1','O24.4','E10','E11','E12','E13')
	THEN 'E14' ELSE icd10_code END AS icd10code,icd10_code
    FROM diagnosis_concept_view
    WHERE icd10_code IN ('E14','E14.3†','E10.1','O24.4','E10','E11','E12','E13')) first_answers
    ...
    JOIN diagnosis_concept_view dcv ON dcv.concept_id = o.value_coded AND dcv.icd10_code IN ('E14','E14.3†','E10.1','O24.4','E10','E11','E12','E13')
...
FROM
    ...
    WHERE
        icd10_code IN ('E04.9','E46','E61','E66.9','D64.9','G62')) first_answers        
        LEFT OUTER JOIN
    (SELECT DISTINCT(p.person_id),dcv.concept_full_name,icd10_code,v.visit_id AS visit_id,p.gender AS gender FROM person p
    INNER JOIN visit v ON p.person_id = v.patient_id AND v.voided = 0
    INNER JOIN encounter e ON v.visit_id = e.visit_id AND e.voided = 0
    ...
    INNER JOIN obs o1 ON o.encounter_id = o1.encounter_id AND o1.obs_group_id = o.obs_group_id AND o1.value_coded = 18
    INNER JOIN concept_name cn ON o.concept_id = cn.concept_id AND cn.concept_name_type = 'FULLY_SPECIFIED' AND cn.name IN ('Coded Diagnosis') AND o.voided = 0 AND cn.voided = 0
    JOIN diagnosis_concept_view dcv ON dcv.concept_id = o.value_coded AND dcv.icd10_code IN ('E04.9','E46','E61','E66.9','D64.9','G62')
    WHERE p.voided = 0) first_concept ON first_concept.icd10_code = first_answers.icd10_code
        LEFT OUTER JOIN
