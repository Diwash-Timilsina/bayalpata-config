...
IFNULL(SUM(CASE WHEN second_concept.gender = 'F' AND second_concept.person_id IS NOT NULL AND second_concept.is_dead THEN 1 ELSE 0  END), 0) AS 'F Deaths',
IFNULL(SUM(CASE WHEN second_concept.gender = 'M' AND second_concept.person_id IS NOT NULL AND second_concept.is_dead THEN 1 ELSE 0  END), 0) AS 'M Deaths',
   combo_id
FROM
    (SELECT 
    ...
    WHERE
		...
		(SELECT DISTINCT
            ...
            INNER JOIN concept_view question ON obs.concept_id = question.concept_id
                AND question.concept_full_name = 'Discharge-Inpatient outcome'
